class BenefitsController < ApplicationController
  ...
  # POST /benefits.json
  def create
	@benefit=current_user.benefits.create(params[:benefit])
    #@benefit = Benefit.new(params[:benefit])

    respond_to do |format|
      if @benefit.save
        format.html { redirect_to @benefit.user, notice: 'Benefit was successfully created.' }
  ...
  def update
    ...
    respond_to do |format|
      if @benefit.update_attributes(params[:benefit])
        format.html { redirect_to @benefit.user, notice: 'Benefit was successfully updated.' }
        format.json { head :ok }
      else
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to @benefit.user }
      format.json { head :ok }
    end
  end
end
