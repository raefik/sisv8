class IklanController < ApplicationController
  ...
  @users=User.all
  @benefit=Benefit.where(:user_id=>"#{@iklan.id}").all
  @available_internships = AvailableInternship.where(:user_id=>"#{@iklan.id}").all
  @requirements=Requirement.where(:user_id=>"#{@iklan.id}").all
  end
